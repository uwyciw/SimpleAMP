...
void HAL_HSEM_FreeCallback(uint32_t SemMask)
{
    ...
    if(pMail != NULL) {
        ...
        if(pMail->length > 0) {
            HAL_UART_Transmit(&huart2, (uint8_t*)pMail->address, pMail->length, HAL_MAX_DELAY);
