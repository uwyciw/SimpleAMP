...
__no_init SAMP_HANDLE_T sampHandle;
...
void SimpleAMP_Init(void)
{
    ...
    for(int i = 0; i < MAIL_NUM; i++) {
        ...
        mails[i].length = 0;
        mails[i].type = SAMP_MAIL_TYPE_RO;  // CM7发送给CM4的数据，CM4只读
    }
}
...
{
    // CM4处理完数据后通知CM7，现在可以回收邮箱
    static unsigned int lastUsedIndex = 0;
    
    // 回收已处理的邮箱，使其可以重新使用
    SAMPReclaimMails(&sampHandle, &lastUsedIndex);
...
/* USER CODE END Boot_Mode_Sequence_2 */
  ...
  {
    /* USER CODE BEGIN 3 */
    char inputStr[MAX_DATA_LEN];
    HAL_StatusTypeDef status;
    
    // 从USART3接收输入字符串
    memset(inputStr, 0, sizeof(inputStr));
    status = HAL_UART_Receive(&huart3, (uint8_t*)inputStr, MAX_DATA_LEN-1, HAL_MAX_DELAY);
    ...
    if(status == HAL_OK) {
        ...
        if(pMail != NULL) {
            // 复制数据到邮箱缓冲区
            strncpy((char*)pMail->address, inputStr, MAX_DATA_LEN-1);
            pMail->length = strlen(inputStr);
